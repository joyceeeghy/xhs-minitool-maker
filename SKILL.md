...
> **流程可裁剪**：六阶段是全量模板，不是每步必走。纯工具类选题（无视觉/3D 需求，如计算器）跳过阶段三、四，只跑 大纲 → 开发 → 物料（万物计算器实证：当天两版交付）。
...
### 阶段三：AI 生图
先按「后端探测顺序」选定当前环境可用的生图后端（不假设宿主有特定文生图工具），再批量生成：历代形态对比墙、穿越时空同框、文物风格化渲染、封面底图。
后端探测顺序、各后端参数映射、提示词配方、封面文字排版纪律（图内不生成正文文字，用 Pillow 后期排版）见 [references/image-prompts.md](references/image-prompts.md)。
...
- **结构**：业务数据（单位表 / 牌价 / 文案配置）独立成 `assets/data.js` 挂 `window`，与逻辑 `main.js` 分离——便于脚本自测和后续更新数据。
- **逻辑门禁**：计算/换算类工具把纯逻辑写成 node 自测脚本（无 DOM 依赖的函数直接断言；有 DOM 依赖的打桩后切片 eval），覆盖边界值（除零、空输入、隐式乘、极大/极小数的显示格式）。参考实证：万物计算器 `test/logic-test.js`。
- **离线数据时效**：内置数据会过期（如汇率牌价），必须给「自定义输入」兜底，让用户手动填当日值。

### 阶段六：发布物料
小红书所需：标题、≤14 字描述、2:3 PNG 封面（1024×1536，已验证可发布）、1:1 ≤5MB PNG 工具图标、发布文案。
