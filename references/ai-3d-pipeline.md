# AI 3D 模型管线（Meshy + Kimi WebBridge）

## 为什么需要 WebBridge

Meshy（及多数网页 3D 工具）没有公开下载 API，且浏览器的程序化文件上传会被 CDP 拦截。Kimi WebBridge 让 AI 驱动**用户自己登录态下的真实 Chrome**：点击、上传、抓包、截图。这是整个 3D 管线的前提。
...
1. **先生成参考图**：用 image_generation 生成「正面全身、A/T pose、服装配饰齐全、纯色背景」的人物/物件图。用户要求不要白模——模型必须穿衣服、带配饰。
...
2. **24 帧转台兜底**：若 `<img>` 路径仍失败，自动切换到预渲染帧动画——本地用 WebBridge 驱动渲染页逐帧渲染 24 张 JPEG（`assets/render_turntable.html` 模板 + `scripts/render_turntable.py` 驱动），小工具内以「自动旋转 + 拖动转身」的帧动画呈现。24 帧 480×552 JPEG 仅约 200KB。
...
- 本地 http.server + WebBridge 截图确认 AI 模型默认加载（不是程序化回退）。
